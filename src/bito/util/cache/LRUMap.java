 ...
 * {@link bito.apache.map.MapIterator MapIterator} or {@link #entrySet()} iterator.
 * <p>
 ...
 * @version $Id: LRUMap.java 1533984 2013-10-20 21:12:51Z tn $
 */
public class LRUMap<K, V> extends AbstractLinkedMap<K, V> implements Map<K, V>, Serializable, Cloneable
{
	...
	protected void addMapping(final int hashIndex, final int hashCode, final K key, final V value)
	{
		if (isFull())
		{
			...
			if (scanUntilRemovable)
			{
				...
				if (reuse == null)
				{
					throw new IllegalStateException("Entry.after=null, header.after"
						+ header.after
							+ " header.before"
							+ header.before
							+ " key="
	 ...
	 * @param value  the value to add
	 */
	protected void reuseMapping(final LinkEntry<K, V> entry, final int hashIndex, final int hashCode, final K key,
		final V value)
	{
		...
		try
		{
			...
			if (loop == null)
			{
				throw new IllegalStateException("Entry.next=null, data[removeIndex]="
					+ data[removeIndex]
						+ " previous="
						+ previous
						+ " key="
		...
		{
			throw new IllegalStateException("NPE, entry="
				+ entry
					+ " entryIsHeader="
					+ (entry == header)
					+ " key="
