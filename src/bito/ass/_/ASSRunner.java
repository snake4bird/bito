...
public class ASSRunner extends bito.ass.ASSRunner implements ProcThreadManager, ProcThreadMonitor
{
	...
	//
	private String systeminitscript_filename = null;
	private String systeminitscript = null;
	private String initscript_filename = null;
	...
	private boolean refreshInitScript()
	{
		try
		{
			...
			if (systeminitscript_filename != null && systeminitscript_filename.length() > 0)
			{
				...
				if (initscript != null && !initscript.equals(systeminitscript))
				{
					...
					// force execute system.init.script
					execute_inner("ass.runner.init", systeminitscript_filename, null);
				}
			}
		}
		catch(Exception e)
	...
	{
		systeminitscript_filename = initscript_filename;
	}

	public void setCommonIncludeScriptFilename(String initscript_filename)
	{
		this.initscript_filename = initscript_filename;
	}
		...
		while(pknames.hasMoreElements())
		{
			...
			if (pkn instanceof String)
			{
				...
				if (pkname.startsWith("data.proc.")
					&& pkname.endsWith(".sql.file")
						&& ps.getProperty(pkname, "").length() > 0)
				{
					String procname = pkname.substring(0, pkname.length() - ".sql.file".length());
					ProcThread.Args args = new ProcThread.Args(SystemConfig.get(procname + ".sql.file"),
	...
	public void procThreadError(ProcThread pt, String message, Throwable e)
	{
		log.error("thread '"
			+ pt.getName()
				+ "' error"
				+ (message != null && message.length() > 0?(", " + message):"."), e);
		synchronized(procthreads_running)
		{
	...
	private Object execute_inner(String proc_name, String sqlfiles, Map parameters) throws Exception
	{
		...
		ProcThread.Args args = new ProcThread.Args(sqlfiles, null, 0, 0, params, initscript);
	...
	private void synchronizeProcThreads()
	{
		...
		synchronized(procthreads_running)
		{
			...
			while(dpesi.hasNext())
			{
				...
				if ((last_error_occurs_time == null || System.currentTimeMillis() > (last_error_occurs_time.longValue() + error_retry_interval))
					&& args.sqlfiles != null
