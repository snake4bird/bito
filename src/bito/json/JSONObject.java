...
public class JSONObject implements Map
{
	...
	{
		myHashMap = new LinkedHashMap(map);
	}
	...
	public Object get(String key) throws NoSuchElementException
	{
		...
		if (o == null)
		{
			throw new NoSuchElementException("JSONObject[" + quote(key, false) + "] not found.");
		}
		return o;
	}
	...
	public boolean getBoolean(String key) throws ClassCastException, NoSuchElementException
	{
		...
		throw new ClassCastException("JSONObject[" + quote(key, false) + "] is not a Boolean.");
	}
	...
	public double getDouble(String key) throws NoSuchElementException, NumberFormatException
	{
		...
		throw new NumberFormatException("JSONObject[" + quote(key, false) + "] is not a number.");
	}
	...
	public JSONArray getJSONArray(String key) throws NoSuchElementException
	{
		...
		throw new NoSuchElementException("JSONObject[" + quote(key, false) + "] is not a JSONArray.");
	}
	...
	public JSONObject getJSONObject(String key) throws NoSuchElementException
	{
		...
		throw new NoSuchElementException("JSONObject[" + quote(key, false) + "] is not a JSONObject.");
	}
	 ...
	 */
	public static String quote(String string, boolean wchar_encode)
	{
		...
		for(i = 0; i < len; i += 1)
		{
			...
			case '\r':
				...
				break;
			default:
				if (c < ' ' || (c >= 128 && c <= 255) || (c > 255 && wchar_encode))
				{
	...
	public String toString()
	{
		...
		while(keys.hasNext())
		{
			...
			if (o != null)
			{
				sb.append(quote(s, true));
				sb.append(':');
				if (o instanceof String)
				{
					sb.append(quote((String)o, true));
				}
	...
	{
		JSON.Format jf = new JSON.Format();
		jf.indent = indentFactor;
		return toString(0, jf);
	}

	public String toString(JSON.Format f)
	{
		return toString(0, f);
	}
	 ...
	 *  representation of the object, beginning with '{' and ending with '}'.
	 */
	String toString(int indent, JSON.Format f)
	{
		StringBuffer sb = new StringBuffer();
		int i;
		Iterator keys = keys();
		...
		{
			prepad += f.indent_separator;
		}
		indent += f.indent;
		String pad = prepad;
		for(; i < indent; i += 1)
		{
			pad += f.indent_separator;
		}
		i = 0;
		sb.append(f.object_head_separator);
		while(keys.hasNext())
		{
			...
			if (o != null)
			{
				if (i == 0)
				{
					sb.append(f.before_first_field_separator);
				}
				else
				{
					sb.append(f.field_separator);
				}
				sb.append(pad);
				sb.append(quote(s, f.wchar_encode));
				sb.append(f.kv_separator);
				if (o instanceof String)
				{
					sb.append(quote((String)o, f.wchar_encode));
				}
				else if (o instanceof Number)
				{
					sb.append(numberToString((Number)o));
				...
				{
					sb.append(((JSONObject)o).toString(indent, f));
				}
				else if (o instanceof JSONArray)
				{
					sb.append(((JSONArray)o).toString(indent, f));
				}
				else
				{
					sb.append(o.toString());
		...
		if (i > 0)
		{
			sb.append(f.after_last_field_separator);
			if (f.after_last_field_separator.endsWith("\n"))
			{
				sb.append(prepad);
			}
		}
		sb.append(f.object_tail_separator);
		return sb.toString();
	}
