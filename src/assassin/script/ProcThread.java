...
public class ProcThread extends Thread
{
	public static class Args
	{
		...
		final String init_script_filename;
		final String string;

		public Args(String sqlfiles, String schedule, long start_delay, long min_interval,
			Map<String, Object> parameters, String init_script, String init_script_filename)
		{
			...
			this.init_script_filename = init_script_filename;
			{
				string = sqlfiles
					+ "\r\n"
						+ schedule
	...
	public ProcThread(ProcThreadMonitor ptmonitor, ProcThreadManager ptmanager, String procname, Args args)
	{
		...
		this.lastArgs = null;
		this.args = args;
		this.asse = new ASScriptEngine();
		this.asse.set("ptmanager", ptmanager);
		this.asse.set("procname", procname);
	}
				...
				if (args != lastArgs
					&& (lastArgs == null
						|| lastArgs.schedule == null
							...
							|| lastArgs.start_delay != args.start_delay
							|| lastArgs.min_interval != args.min_interval))
				{
				...
				else if (System.currentTimeMillis() >= dt_nextrun_starttime)
				{
					...
					try
					{
								...
								&& (lastArgs == null || !args.parameters.equals(lastArgs.parameters)))
						{
							...
							{
								asse.set(kv.getKey(), kv.getValue());
							}
						}
						if (args != lastArgs
						...
						{
							asse.eval(args.init_script);
						}
						if (args != lastArgs
							&& args.init_script_filename != null
								&& args.init_script_filename.length() > 0
								&& (lastArgs == null
									|| !args.init_script_filename.equals(lastArgs.init_script_filename)))
						{
							asse.exec(args.init_script_filename);
						}
						String fn = args.sqlfiles;
						if (fn != null && fn.length() > 0)
						{
							asse.set("error", null);
							lastResult = asse.exec(args.sqlfiles);
							ptmonitor.procThreadInfo(this, "returned " + lastResult);
							if (asse.get("error") != null)
							{
								ptmonitor.procThreadError(this,
									"error occurs when sql running." + asse.get("error"),
									null);
							}
						}
					}
