...
{
	public static final Map<String, String> db2_message = bito.util.E.V().newMapSortedByAddTime();
	static
	{
		try
		{
			...
			try
			{
				message_map_string = new String(bito.util.E.V().readBytes(is));
			}
			finally
			{
				is.close();
			}
			db2_message.putAll((Map<String, String>)new ASScriptEngine().eval(message_map_string));
		}
		catch(Exception e)
		{
			e.printStackTrace();
