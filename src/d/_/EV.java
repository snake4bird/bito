...
public class EV extends EVD
{
	...
	private void tailingfile(String filename)
	{
	...
	private static EscapeSequence es = null;

	public d.EscapeSequence getEscapeSequence()
	{
		if (es == null)
		{
			es = new EscapeSequence();
		}
		return es;
	}
	...
	public d.FileAppender newFileAppender(String filename, long maxfilesize, int maxbackindex)
	{
		return new FileAppender(filename, maxfilesize, maxbackindex);
	}
	...
	public d.SQLRunner newSQLRunner(String name)
	{
		return new d._.asql.SQLRunner(name);
	}

	public d.HashTree newHashTree()
	{
		return newHashTree(Object.class);
	}

	public d.HashTree newHashTree(Class componentType)
	{
		return new d._.HashTree(componentType);
	}
			...
			while(count > 0)
			{
				...
				if (cachestamp == null
					|| System.currentTimeMillis() > cachestamp.longValue()
						+ 1000L
							* SystemConfig.getLong("read.file.cache.seconds", 1800)
		...
		synchronized(writefilecachestamp)
		{
			...
			while(count > 0)
			{
				...
				if (cachestamp == null
					|| System.currentTimeMillis() > cachestamp.longValue()
						+ 1000L
							* SystemConfig.getLong("write.file.cache.seconds", 1800)
