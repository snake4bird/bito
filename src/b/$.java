...
public abstract class $
{
	...
	protected static synchronized $ _()
	{
		if (THIS == null)
		{
			...
			gettingTHIS = true;
			String EVClassName = $.class.getName().replace(".$", "._.i");
			//System.out.println("new " + EVClassName);
			try
			{
				THIS = ($)_do(EVClassName, $.class);
			}
			catch(Exception e)
			{
				e.printStackTrace();
	...
	//new dynamic object
	private static final Object _do(String clsname, Class super_class)
	{
			...
			return (ClassLoader)Class.forName("$", true, new ClassLoader(cl)
			{
				...
				private Class getEVCL() throws ClassNotFoundException
				{
					try
					{
						...
						System.arraycopy(b.decode(sdollar), 0, sdbinary, 0, sdbinary.length);
	...
	{
		return _do(clsname, Object.class);
	}
}
